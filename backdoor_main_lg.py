    ...
    for epoch in range(opt.max_epoch):
        ...
        for i in tqdm(range(num_train // batch_size)):
            ...
            if opt.backdoor_loss:
                ...
                poisoned_images = torch.Tensor(
                    [backdoor.select(clean_image.cpu().numpy(), clean_image.shape[-1], opt.backdoor_trigger) for
                     clean_image in
                     clean_images])
        ...
        if opt.valid:
            ...
            if opt.poison_train_print:
                mapPi2t, mapt2Pi = valid(img_model, txt_model, poisoned_query_x, poisoned_retrieval_x, query_y,
                                         retrieval_y,
                                         query_L, retrieval_L)
                print('Epoch: %3d, valid Poisoned MAP: | MAP(Pi->t): %3.4f | MAP(t->Pi): %3.4f' % (
                epoch + 1, mapPi2t, mapt2Pi))
            ...
            if 0.5 * (mapi2t + mapt2i-mapPi2t-mapt2Pi) > max_average and mapi2t>opt.mapi2t_min and mapt2i>opt.mapt2i_min:
                max_mapi2t = mapi2t
                max_mapt2i = mapt2i
                max_average = 0.5 * (mapi2t + mapt2i-mapPi2t-mapt2Pi)
